...
global.fetch = vi.fn((input: RequestInfo | URL, _init?: RequestInit) => {
  ...
  if (input.toString().includes('/status')) {
    ...
    if (input.toString().includes('/v1/me/top/tracks')) {
        ...
        mockResponseData = { display_name: "Mock User", id: "mockid", email: "mock@example.com", images: [] }; // Example for getMe
  ...
  const res = {
    ...
    text: () => Promise.resolve(typeof mockResponseData === 'string' ? mockResponseData : JSON.stringify(mockResponseData)),
